...
- **me- (beside/near):** **`ADJ`** (Adjacent) or **`PROX`** (Proximal).
    
- **pe- (distant/unalignment):** **`DIST`** (Distal) or **`ANTIPROX`**.

- **ko- (pro / favor):** **`PRO`** (Pro-prefix).
    
- **-gi (concept/abstract):** **`ABST`** (Abstract).
    
- **-wi (theory/ology):** **`THEOR`** (Theoretical).
...
- **gen- (trans/inter):** **`INTER`** (Inter-prefix).
